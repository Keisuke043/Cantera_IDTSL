...
if __name__ == '__main__':
    ...
    read_hdf_dic = {'DTU':'Results_IDT/Results_IDT_DTU.h5', 'Lu':'Results_IDT/Results_IDT_Lu.h5'}

    df_DTU = read_IDT_hdf(read_hdf_dic['DTU'])
    df_Lu  = read_IDT_hdf(read_hdf_dic['Lu'])

    fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(10,7))
    ax2 = ax.twiny()

    df_DTU.plot(kind='line', ax=ax, x='T_inv', y='tau1_p1.0', lw=2, c='r', ls='--', label='')
    df_DTU.plot(kind='line', ax=ax, x='T_inv', y='tau2_p1.0', lw=2, c='r', ls='-',  label='DTU')

    df_Lu.plot(kind='line', ax=ax, x='T_inv', y='tau1_p1.0', lw=2, c='b', ls='--', label='')
    df_Lu.plot(kind='line', ax=ax, x='T_inv', y='tau2_p1.0', lw=2, c='b', ls='-',  label='Lu')
