...
if flag_save_history:
    savedir_timeHist = './{}/timeHist_{}_phi{}_hdf'.format(savedir, key_chem, equivalence_ratio)
    subprocess.call(['mkdir','-p', savedir_timeHist])
...
for caseNumber in range(caseNumber_sta, caseNumber_end+1):
    ...
    if(flag_save_history):
        ...
        timeHistory_state.write_hdf('{}/timeHistory_p{}_T{}.h5'.format(savedir_timeHist, p, T), \
                     cols=('t', 'T', 'P', 'X', 'heat_release_rate'), group='p{}_T{}'.format(p, T))

    # print('Case = {:>/4}, p,T = {:>7} {:>6}, tau = {:e} '.format(caseNumber, p, T, tau_T_grad))
    print('Case = {}, p,T = {} {}, tau = {} '.format(caseNumber, p, T, tau_T_grad))
